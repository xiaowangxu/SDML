...
export const ALL_INPUTS_TYPES = {
	...
	quat: {
		default: (val) => { return val },
		datatype: () => ExpTypes.base(ExpTypes.quat),
		diff: (dep) => {
			return `!this.i.${dep}.equals(i.${dep})`;
		}
	},
	...
	'string-arr': {
		default: (val) => { return val },
		datatype: () => ExpTypes.array(ExpTypes.base(ExpTypes.string)),
	},
	'bool-arr': {
		default: (val) => { return val },
		datatype: () => ExpTypes.array(ExpTypes.base(ExpTypes.bool)),
	},
	'vec3-arr': {
		default: (val) => { return val },
...
export class Types {
	...
	static ANY = Symbol("accept any types");
}
...
export class Collection {
	...
	set(match_type, type, nodes) {
		const match_obj = this.add_Class(match_type);
		if (type in match_obj) match_obj[type] = new Set(nodes);
		else {
